...
This project uses a true distributed database-per-service architecture. The Auth, Events, Booking, Payment, and Ticket services each own their own Supabase database.

No service directly reads or writes another service database. Services communicate through Vercel API routes and future event messages. This keeps ownership clear and avoids tight database coupling between services, so the system is distributed rather than centralized.
...
Foreign keys work inside a single database. In this architecture, each service has a separate Supabase project/database, so cross-database foreign keys are intentionally not used.
